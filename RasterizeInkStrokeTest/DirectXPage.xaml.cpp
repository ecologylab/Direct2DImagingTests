...
using namespace RasterizeInkStrokeTest;
...
void DirectXPage::ShowInkStrokeExamples(void)
{
	Grid^ grid = (Grid^) Application::Current->Resources->Lookup("inkStrokes");
	if (grid != nullptr)
	{
		while(grid->Children->Size > 0)
		{
			Canvas^ element = (Canvas^) grid->Children->GetAt(0);
			grid->Children->RemoveAt(0);
			element->Margin = Thickness(20);
			StrokeExamples->Children->Append(element);
		}
	}
}
